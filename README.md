# PetClinic (.NET 10 + Blazor Server) | A .NET Port of Spring PetClinic
This repository is a non-official .NET 10 port of [Spring PetClinic](https://github.com/spring-projects/spring-petclinic), built with Blazor Server and Entity Framework Core.

Its purpose is to reproduce the original application's core functionality, domain model, and user experience, while leveraging modern .NET architectural patterns and practices such as clean layering, async/await, validation, automated testing, containerization with Docker, localization, and caching strategies.


> ⚠ **Non-Official Port Notice**
>
> This repository is a **.NET 10 + Blazor Server port** of the original
> [Spring PetClinic](https://github.com/spring-projects/spring-petclinic).
>
> It is an independent reimplementation of the application's functionality
> and architecture concepts within the .NET ecosystem.
>
> This project is **not affiliated with, endorsed by, or maintained by**
> the Spring Team, VMware, or Broadcom.
>
> The original Spring PetClinic project is licensed under the Apache License 2.0.


## Legal Notice

This project is an educational and technical port of Spring PetClinic
to the .NET platform.

It reimplements the domain model, UI flows, and use cases using:
- .NET 10
- Blazor Server
- Entity Framework Core

No official relationship exists between this repository and the
Spring PetClinic maintainers
